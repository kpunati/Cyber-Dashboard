...
function cacheLabel(data: CyberDashboardData) {
  if (Object.values(data.sourceStatus).some(source => source.status === 'fallback')) return 'Preview fallback';
  ...
  return (
    <div className="min-h-screen overflow-hidden bg-[#030707] text-white">
      ...
      <main className="relative mx-auto max-w-[1640px] p-3 md:ml-24 md:p-4">
        ...
        {/* Header Section */}
        <section className="panel relative mb-4 overflow-hidden rounded-lg border border-slate-700/70 bg-[#05090a]/95 p-0" id="overview">
          <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-amber-300/70 to-transparent" />
          <div className="grid grid-cols-1 lg:grid-cols-[88px_1fr_330px_410px]">
            <div className="flex items-center justify-center border-b border-slate-700/60 bg-black/20 p-4 lg:border-b-0 lg:border-r">
              <div className="relative flex h-14 w-14 items-center justify-center rounded-xl border border-amber-400/40 bg-amber-400/10 text-amber-300 shadow-lg shadow-amber-500/10">
                <svg viewBox="0 0 48 48" className="h-10 w-10" fill="none" stroke="currentColor" strokeWidth="2.2">
                  <path d="M24 4 39 10v12c0 10-6 17-15 21C15 39 9 32 9 22V10L24 4Z" />
                  <path d="M16 25h5l3-9 4 17 3-8h5" />
                </svg>
              </div>
            </div>
            <div className="flex flex-col justify-center border-b border-slate-700/60 p-5 lg:border-b-0 lg:border-r">
              <h1 className="text-2xl font-black uppercase tracking-[0.08em] text-amber-300 md:text-3xl">Cyber Threat Pulse</h1>
        ...
        {/* Filters */}
        <section className="mb-4 rounded-lg border border-amber-500/20 bg-black/30 p-3">
          <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.22em] text-amber-300">Intelligence filters</p>
              <p className="mt-1 text-xs text-slate-400">Filters apply to visible lists and keep all processing client-side.</p>
            </div>
              ...
              <div className="flex flex-wrap gap-2">
                {severityFilters.map(filter => (
                  <button
                    ...
                    className={`rounded border px-3 py-1.5 text-[0.68rem] font-semibold uppercase tracking-[0.12em] transition ${
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[1fr_1.45fr_1fr] gap-3 mt-3">
          {/* New Vulnerabilities */}
          ...
          {/* Supply Chain Watch */}
          <div id="supply-chain">
            <SupplyChainWatch items={filteredAdvisories} />
          </div>
